...
2. Sends a `repo:diff-peer` request to get the diff information
...
1. **Peer Selection**: Muninn app sends a `peer:select` message with peer data
2. **Event Routing**: Message is routed to the appropriate handler via the events table
3. **Diff Request**: Emacs sends a `repo:diff-peer` request with project and peer information
