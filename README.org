...
* Kawa Code - a low noise collaboration toolset for Emacs     :noexport:
...
Other components of the Kawa Code ecosystem are also in active state of development, so please
follow us on github or our website.

Please see the README-Development.md for more details.

* Quick Feature Overview
Kawa Code highlights the code intersections between your working copy and other team member's.
...
First please download and install Kawa Code from [[https://codeawareness.com][Kawa Code website]].
...
3. **Force loading of compiled version (optional):**
   ...
   ```elisp
   (defun dotspacemacs/user-config ()
     ;; Kawa Code Configuration (compiled version)
     (let ((code-awareness-path "/home/username/ca.emacs/"))
       (when (file-exists-p code-awareness-path)
         (add-to-list 'load-path code-awareness-path)
...
* Contributing
Contributions are very much welcome, but should fit the general scope and style of Kawa Code.
